...
function addDriver(tx) {
  return getParticipantRegistry("it.freighttrust.com.Driver").then(function (
    participantRegistry
  ) {
    // Create the driver
    var factory = getFactory();
...
function createDriverPassport(create) {
  ...
  var passport = getFactory().newResource(
    "it.freighttrust.com",
    "DriverPassport",
    create.Id
  );
  ...
  return getAssetRegistry("it.freighttrust.com.DriverPassport")
    ...
    .then(function () {
      ...
      var notification = getFactory().newEvent(
        "it.freighttrust.com",
        "CreateDriverPassportNotification"
      );
...
function updateDriverPassport(update) {
  ...
  return getAssetRegistry("it.freighttrust.com.DriverPassport").then(function (
    assetRegistry
  ) {
    // emit a notification that an update has occured
    var notification = getFactory().newEvent(
      "it.freighttrust.com",
      "UpdateDriverPassportNotification"
    );
...
function changeDriverCarrier(tx) {
  ...
  return getParticipantRegistry("it.freighttrust.com.Driver").then(function (
    ...
    var notification = getFactory().newEvent(
      "it.freighttrust.com",
      "ChangeDriverCarrierNotification"
    );
...
function removeDriverCarrier(tx) {
  ...
  return getParticipantRegistry("it.freighttrust.com.Driver").then(function (
    ...
    var notification = getFactory().newEvent(
      "it.freighttrust.com",
      "RemoveDriverCarrierNotification"
    );
...
function listLoad(listing) {
  return getAssetRegistry("it.freighttrust.com.Load").then(function (
    assetRegistry
  ) {
    // create the new load listing
    var load = getFactory().newResource(
      "it.freighttrust.com",
      "Load",
      listing.Id
    );
...
function submitQuote(tx) {
  ...
  return getAssetRegistry("it.freighttrust.com.Quote").then(function (
    assetRegistry
  ) {
...
function assignDriverToLoad(tx) {
  ...
  return getAssetRegistry("it.freighttrust.com.Load").then(function (
    ...
    var notification = getFactory().newEvent(
      "it.freighttrust.com",
      "AssignDriverToLoadNotification"
    );
...
function loadPickup(tx) {
  ...
  var detentionMinutes = getDetentionMinutes(
    tx.Load.ScheduledPickupDate,
    tx.Load.OriginArrivalDate,
    tx.Load.PickupDate,
    tx.Load.Carrier.DetentionGraceHours
  );
  ...
  return getAssetRegistry("it.freighttrust.com.Load")
    ...
    .then(function (detentionRegistry) {
      ...
      // TODO: id generation
      var det = getFactory().newResource(
        "it.freighttrust.com",
        "Detention",
        "D_" + tx.Load.Id + "_" + "Origin"
      );
      det.Type = "Origin";
      det.Load = tx.Load;
...
function loadDropOff(tx) {
  ...
  // notification
  var detentionMinutes = getDetentionMinutes(
    tx.Load.ScheduledDropOffDate,
    tx.Load.DestinationArrivalDate,
    tx.Load.DropOffDate,
    tx.Load.Carrier.DetentionGraceHours
  );
  var detention = isDetention(detentionMinutes);
...
function createDetention(tx) {
  // Create the resource
  var factory = getFactory();
  var detention = factory.newResource(
    "it.freighttrust.com",
    "Detention",
    tx.Id
  );
