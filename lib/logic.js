...
/**
 * Add a carrier
 * @param {it.freighttrust.com.CreateCarrier} tx - the transaction to be
 *     processed
 * @transaction
 */
function createCarrier(tx) {
  ...
  return getParticipantRegistry('it.freighttrust.com.Carrier')
      ...
      .then(function() {
        ...
        var notification = getFactory().newEvent('it.freighttrust.com',
                                                 'CreateCarrierNotification');
...
/**
 * Add a shipper
 * @param {it.freighttrust.com.CreateShipper} tx - the transaction to be
 *     processed
 * @transaction
 */
...
/**
 * Add a receiver
 * @param {it.freighttrust.com.CreateReceiver} tx - the transaction to be
 *     processed
 * @transaction
 */
...
function addDriver(tx) {
  ...
  return getParticipantRegistry('it.freighttrust.com.Driver')
      .then(function(participantRegistry) {
        ...
        var factory = getFactory();
        var driver =
            factory.newResource('it.freighttrust.com', 'Driver', tx.Id);
 ...
 * @param {it.freighttrust.com.CreateDriverPassport} create - the passport
...
function createDriverPassport(create) {
  ...
  var passport = getFactory().newResource('it.freighttrust.com',
                                          'DriverPassport', create.Id);
  ...
  return getAssetRegistry('it.freighttrust.com.DriverPassport')
      ...
      .then(function() {
        // emit a notification
        var notification = getFactory().newEvent(
            'it.freighttrust.com', 'CreateDriverPassportNotification');
 ...
 * @param {it.freighttrust.com.UpdateDriverPassport} update - the update to be
 *     processed
 * @transaction
 */
function updateDriverPassport(update) {
  ...
  if (update.DrugAndAlchoholCompliance != null) {
    save = true;
    update.Passport.DrugAndAlchoholCompliance =
        update.DrugAndAlchoholCompliance;
  }
  ...
  return getAssetRegistry('it.freighttrust.com.DriverPassport')
      .then(function(assetRegistry) {
        ...
        var notification = getFactory().newEvent(
            'it.freighttrust.com', 'UpdateDriverPassportNotification');
...
/**
 * Track update driver
 * @param {it.freighttrust.com.ChangeDriverCarrier} tx - the transaction to be
 *     processed
 * @transaction
 */
function changeDriverCarrier(tx) {
  ...
  return getParticipantRegistry('it.freighttrust.com.Driver')
      .then(function(participantRegistry) {
        // emit a notification that an update has occured
        var notification = getFactory().newEvent(
            'it.freighttrust.com', 'ChangeDriverCarrierNotification');
 ...
 * @param {it.freighttrust.com.RemoveDriverCarrier} tx - the transaction to be
 *     processed
 * @transaction
 */
function removeDriverCarrier(tx) {
  ...
  return getParticipantRegistry('it.freighttrust.com.Driver')
      .then(function(participantRegistry) {
        // emit a notification that an update has occured
        var notification = getFactory().newEvent(
            'it.freighttrust.com', 'RemoveDriverCarrierNotification');
...
function listLoad(listing) {
  return getAssetRegistry('it.freighttrust.com.Load')
      .then(function(assetRegistry) {
        ...
        var load =
            getFactory().newResource('it.freighttrust.com', 'Load', listing.Id);
...
function submitQuote(tx) {
  ...
  return getAssetRegistry('it.freighttrust.com.Quote')
      .then(function(assetRegistry) {
        ...
        var notification =
            getFactory().newEvent('it.freighttrust.com', 'QuoteNotification');
 ...
 * @param {it.freighttrust.com.AssignDriverToLoad} tx - the transaction to be
 *     processed
 * @transaction
 */
function assignDriverToLoad(tx) {
  ...
  return getAssetRegistry('it.freighttrust.com.Load')
      .then(function(assetRegistry) {
        ...
        var notification = getFactory().newEvent(
            'it.freighttrust.com', 'AssignDriverToLoadNotification');
...
function isDetention(detentionMinutes) { return detentionMinutes > 0; }

// Gets the amount of detention minutes based on the scheduled time + grace
// period compared to the actual time the load was picked up or dropped off.
...
/**
 * Called by the Driver to log when they have arrived at the Origin and are
 * waiting to pick up the Load.
 * @param {it.freighttrust.com.OriginArrival} tx - the transaction to be
 *     processed
 * @transaction
 */
...
function loadPickup(tx) {
  ...
  // if driver arrived by ScheduledPickupDate and picked up after
  // carrier.DetentionGraceHours past ScheduledPickupDate, emit detention
  // notification
  var detentionMinutes = getDetentionMinutes(
      tx.Load.ScheduledPickupDate, tx.Load.OriginArrivalDate,
      tx.Load.PickupDate, tx.Load.Carrier.DetentionGraceHours);
  ...
  return getAssetRegistry('it.freighttrust.com.Load')
      ...
      .then(function(detentionRegistry) {
        ...
        // TODO: id generation
        var det = getFactory().newResource('it.freighttrust.com', 'Detention',
                                           'D_' + tx.Load.Id + '_' +
                                               'Origin');
        det.Type = 'Origin';
        det.Load = tx.Load;
...
/**
 * Called by the Driver when they have arrived at the Destination and are
 * waiting to drop off the Load.
 * @param {it.freighttrust.com.DestinationArrival} tx - the transaction to be
 *     processed
 * @transaction
 */
function destinationArrival(tx) {
  ...
  return getAssetRegistry('it.freighttrust.com.Load')
      ...
      .then(function() {
        // emit a notification that a load event has occured
        var notification = getFactory().newEvent(
            'it.freighttrust.com', 'DestinationArrivalNotification');
...
function loadDropOff(tx) {
  ...
  // if driver arrived by ScheduledDropOffDate and picked up after
  // carrier.DetentionGraceHours past ScheduledDropOffDate, emit detention
  // notification
  var detentionMinutes = getDetentionMinutes(
      tx.Load.ScheduledDropOffDate, tx.Load.DestinationArrivalDate,
      tx.Load.DropOffDate, tx.Load.Carrier.DetentionGraceHours);
  var detention = isDetention(detentionMinutes);
  return getAssetRegistry('it.freighttrust.com.Load')
      .then(function(assetRegistry) {
        // persist the state of the load
 ...
 * @param {it.freighttrust.com.CreateDetention} tx - the transaction to be
 *     processed
 * @transaction
 */
function createDetention(tx) {
  ...
  var factory = getFactory();
  var detention =
      factory.newResource('it.freighttrust.com', 'Detention', tx.Id);
  detention.Id = tx.Id;
